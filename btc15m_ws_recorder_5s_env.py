...
"""
Polymarket BTC 15m recorder (5s sampling, last 15m only), configured by .env

Features:
- Discover BTC 15m markets via Gamma API (open markets), filter by slug prefix.
- Record ONLY last 15 minutes of each market.
...
Output CSV (daily):
btc15m_ticks_YYYY-MM-DD.csv with columns:
...
SESSION.headers.update({"User-Agent": "btc15m-ws-recorder/1.0"})
...
def discover_btc15m_markets(now_ts: int, lookahead_hours: int, timeout_read: int) -> List[MarketInfo]:
    """
    Real-time window strategy:
    - Construct slugs by 15m grid around now (start_ts in seconds).
    ...
    # Generate slugs: from now-2h to now+lookahead_hours, step 15m
    back_hours = 2
    step = 15 * 60
    ...
    for ts in range(start_ts, end_ts + 1, step):
        ...
        # Prefer endDate from Gamma; fallback to start_ts + 15m.
...
class Recorder:
    def __init__(
        ...
        self.last15m_only = last15m_only
    ...
    def _csv_path_for_ts(self, ts: int) -> str:
        return os.path.join(self.outdir, f"btc15m_ticks_{utc_day_str(ts)}.csv")
    ...
    def compute_active_tokens(self, markets: List[MarketInfo], now_ts: int) -> List[str]:
        ...
        for m in markets:
            if self.last15m_only:
                start_ts = m.end_ts - 15 * 60
                ...
                async with websockets.connect(
                    ...
                    while True:
                        ...
                        if now - self._last_refresh_ts >= self.refresh_sec:
                            markets = discover_btc15m_markets(
                                now_ts=now,
                            ...
                            if new_active != self.active_tokens:
...
def main():
    ...
    last15m_only = get_bool(os.getenv("LAST15M_ONLY"), True)
